...
// API 키 설정 (보안을 위해 실제 서비스 시 .env 파일 사용 권장)
const KEYS = {
...
function getSignalData() {
    ...
    return [
        { 
            ITRSC_NM: "오현로20길 교차로 A", 
            LGHT_COL_CD: aIsGreen ? "1" : "2", 
            RESID_TIME: aIsGreen ? GREEN_SEC - elapsed : CYCLE - elapsed, 
            SIMULATED: true 
        },
        { 
            ITRSC_NM: "오현로20길 교차로 B", 
            LGHT_COL_CD: bIsGreen ? "1" : "2", 
            RESID_TIME: bIsGreen ? GREEN_SEC - ((elapsed + RED_SEC) % CYCLE) : CYCLE - ((elapsed + RED_SEC) % CYCLE), 
            SIMULATED: true 
        }
    ];
}

// [API] 실시간/시뮬레이션 신호등 정보
app.get("/api/signal", async (req, res) => {
    try {
        ...
        console.log("⚠️ 서울시 API 연결 실패 -> 시뮬레이션 모드로 전환합니다."); 
    }

    res.json({ 
        SptTrafficLghtResidTime: { 
            RESULT: { CODE: "INFO-000", MESSAGE: "Simulation Mode" }, 
            row: getSignalData() 
        } 
    });
});
...
app.get("/api/stations", async (req, res) => {
    ...
    if (!lat || !lng) return res.status(400).json({ error: "위치 정보 부족" });
    ...
    try {
        ...
        res.status(502).json({ error: "카카오 API 호출 실패" }); 
    }
});
